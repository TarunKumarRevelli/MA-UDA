...
def infer_cyclegan(image_path, checkpoint_path, direction='s2t'):
    ...
    # 2. Load and Fix Image (THE CRITICAL FIX)
    ...
    if image_path.endswith('.npy'):
        ...
        if img_arr.dtype == np.float32 or img_arr.dtype == np.float64:
            if img_arr.max() <= 1.1: # Allow small margin for error
    ...
    # 3. Transform (Resize to 128 to match training!)
    ...
    def to_numpy(tensor):
        ...
        img = (img + 1) / 2  # Denormalize (-1 to 1) -> (0 to 1)
...
# --- RUN IT ---
# Use the file you just verified
