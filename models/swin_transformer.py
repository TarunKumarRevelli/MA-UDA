...
# ... (Imports and previous classes like PatchEmbed, SwinTransformerBlock remain the same) ...

# class SwinTransformerSegmentation(nn.Module):
#     """
#     Swin Transformer-based segmentation model for MA-UDA
#     (Memory Optimized: Only stores necessary attention masks)
#     """
    
#     def __init__(self, img_size=128, patch_size=4, in_chans=3, num_classes=4,
#                  embed_dim=48, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
#                  use_checkpoint=True, window_size=4): 
#         super().__init__()
...
#     def forward(self, x):
#         B, C, H, W = x.shape
#         self.attention_masks = [] # Reset storage
...
#                 # Only save the FIRST attention mask (index 0).
#                 # Your trainer code only accesses attn_masks[0].
#                 # Saving the others wastes HUGE amounts of VRAM.
#                 if len(self.attention_masks) == 0: 
#                     if block.attention_weights is not None:
#                         self.attention_masks.append(block.attention_weights)
                
#                 # Important: If you don't save it, explicitly clear it from the block
#                 # to help the garbage collector
#                 else:
#                     block.attention_weights = None 
