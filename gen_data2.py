...
class GeneratorResNet(nn.Module):
    def __init__(self, input_shape, num_residual_blocks):
        ...
        # --- UPSAMPLING (Matches your cyclegan99.pth) ---
        for _ in range(2):
            out_channels //= 2
            model += [
...
def generate_cyclegan_data():
    ...
    if isinstance(checkpoint, dict):
        # Prioritize G_s2t (Source->Target) or G_AB
        if 'G_s2t' in checkpoint: state_dict = checkpoint['G_s2t']
        elif 'G_AB' in checkpoint: state_dict = checkpoint['G_AB']
